...
if __name__ == '__main__':
    ...
    # 每5轮训练一次
    for iter in range(args.epochs-5, args.epochs):
        loss_locals = []
        if not args.all_clients:
            w_locals = []
            idxs_users = [i for i in range(args.num_users)]
            delete_users = np.random.choice(range(72), math.floor((1-args.frac) * 72), replace=False)
            for del_idx in delete_users:
                for i in range(del_idx * 22, (del_idx+1) * 22):
                    idxs_users.remove(i)
        else:
            idxs_users = [i for i in range(args.num_users)]
