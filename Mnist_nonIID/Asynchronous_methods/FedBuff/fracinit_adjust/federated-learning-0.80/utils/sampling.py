...
# 参考文献6 Non-IID版本
# def mnist_noniid_2(dataset, num_users):
#     """
#     Sample non-I.I.D client data from MNIST dataset
#     :param dataset:
#     :param num_users:
...
def emnist_noniid(dataset, num_users):
    ...
    # divide and assign
    num_items = int(len(dataset)/num_users) * 3
    ...
    for i in range(num_users):
        if i < num_users * 0.5 :
            dict_users[i] = set(np.random.choice(all_idxs_40, num_items, replace=True))
            # all_idxs_40 = list(set(all_idxs_40) - dict_users[i])
        else :
            dict_users[i] = set(np.random.choice(all_idxs_60, num_items, replace=True))
