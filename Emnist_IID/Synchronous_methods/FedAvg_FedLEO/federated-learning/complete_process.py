    ...
    # Get next line from file
        ...
        for i, slice in enumerate(slices):
            ...
            # elif i == 1:
                ...
                satellites[sat_idx].records[visible_idx].com_time_down = tp + tt_downlink # FedLEO开始考虑多颗卫星并行抢占带宽的可能
...
def choose_orbit_source(orbit, current_time):
    ...
    for sat in orbit.satellites:
        if (current_time >= sat.records[sat.visible_idx].start_tstamp) & \
            ...
            if sat.records[sat.visible_idx].range < min_range:
                if min_range != 10000:
    ...
    for sat in orbit.satellites:
        if ((sat.records[sat.visible_idx].start_tstamp - current_time) <= min_waiting_time) & (sat.visited == 0):
            ...
            # print(orbit.source_node_id, "source node 被调整为", sat.plane_id)
            orbit.source_node_id = sat.plane_id
            sat.visited = 1
    return (min_waiting_time + timeline_advancement)


# 轨道上所有卫星只要训练完了，都可以计算能不能Tsum,因此都在候选集中
def choose_orbit_sink(orbit):
    # 组成tc_U tc_D t_wait Ttrain t_h
    ...
    T_sum_min = stk_train_end_stamp - t_n
    for sat in orbit.satellites:
        T_sum_tmp = 0.0
        # 本地模型训练完，才开始考虑能否竞选sink node
        current_time = t_n + sat_to_sat_delay(orbit, orbit.source_node_id, sat.plane_id) + sat_train_time
        # 在时间段之前的情况
        while sat.records[sat.visible_idx].stop_tstamp <= current_time:
            sat.visible_idx += 1
        ...
        T_sum_tmp += sat.records[sat.visible_idx].com_time_up
        T_sum_tmp += sat.records[sat.visible_idx].com_time_down

        if (current_time >= sat.records[sat.visible_idx].start_tstamp) & \
             (current_time <= sat.records[sat.visible_idx].stop_tstamp):
            T_sum_tmp += 0
        else:
            T_sum_tmp += sat.records[sat.visible_idx].start_tstamp - current_time
        
        T_sum_tmp += sat_train_time

        # 最后考虑t_h
        T_sum_tmp += math.ceil(total_planes_cnt/2) * Size_of_model / (Bandwidth_sat2sat * Spectral_efficiency)

        # 恢复原来的可见期索引，上面只是在预测而已
        sat.visible_idx = pre_visible_idx
        # 如果可见期最大 则选作sink node
        if T_sum_tmp < T_sum_min:
            # print(orbit.sink_node_id, "sink node 被调整为", sat.plane_id, T_sum_min, T_sum_tmp)
            T_sum_min = T_sum_tmp
...
def sink_send_model_PS(orbit, current_time):
    # 更新sink node的当前索引
    while orbit.satellites[orbit.sink_node_id].records[orbit.satellites[orbit.sink_node_id].visible_idx].stop_tstamp < current_time:
        orbit.satellites[orbit.sink_node_id].visible_idx += 1
    ...
    # 在sink node的当前可见期之前/之后，任务过早/过晚完成，那么就一直等到sink node的下一个可见期，没有其他的处理策略
    else: 
        orbit.satellites[orbit.sink_node_id].visible_idx += 1
        orbit_end = orbit.satellites[orbit.sink_node_id].records[orbit.satellites[orbit.sink_node_id].visible_idx].start_tstamp + \
...
def vir_train_process(epochs_needed, vir_start_time):
    ...
    while round < epochs_needed:
        ...
        for orbit in orbits:
            ...
            # 这里重点不是为了处理idle状态的卫星，而是防止初始的source node没有做好选择
        ...
        print("Epoch No."+str(round + 1), "training is over, now it is ", t_end)
...
if __name__ == '__main__':
    ...
    stamp1 = stk_train_start_stamp
    stamp2 = vir_train_process(20, stamp1)
