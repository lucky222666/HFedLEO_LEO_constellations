...
def model_will_received(choosed_sat, cur_idx):
    if satellites[choosed_sat].records[cur_idx].com_time_up < satellites[choosed_sat].records[cur_idx].delta_tstamp:
        ...
        if satellites[choosed_sat].records[cur_idx].com_time_up + sat_train_time < satellites[choosed_sat].records[cur_idx].delta_tstamp:
    ...
    if (satellites[choosed_sat].model_received == 1) & (satellites[choosed_sat].model_trained == 1):
        ...
        total_com_time = satellites[choosed_sat].records[cur_idx].com_time_up + satellites[choosed_sat].records[cur_idx].com_time_down
...
def vir_train_judge(choosed_sat): 
    pre_size = len(MoRolla.model_up)
    MoRolla.model_up.add(choosed_sat)
    if len(MoRolla.model_up) == pre_size:
        # print(choosed_sat, "has sended model -> GS")
        return # 该卫星在本轮已经完成卫星模型的发送
    else:
        MoRolla.model_up.remove(choosed_sat)
        
        # print(choosed_sat, "is choosed to send model -> GS")
        
        cur_idx = satellites[choosed_sat].visible_idx
        # 本地训练模型未上传的卫星
        if (satellites[choosed_sat].model_received == 1) & (satellites[choosed_sat].model_trained == 1) & (satellites[choosed_sat].model_sended == 0):
            model_trained_unsended(choosed_sat, cur_idx)
        # 准备接收新一轮全局模型的卫星
        elif (satellites[choosed_sat].model_received == 0) & (satellites[choosed_sat].model_trained == 0) & (satellites[choosed_sat].model_sended == 0):
...
def vir_train_update():
    for sat in satellites:
        if (sat.model_received == 1) & (sat.model_trained == 0): # 为此类卫星选择及时更新模型训练结果
            if (sat.visible_idx > 0) & (sat.records[sat.visible_idx-1].start_tstamp+sat.records[sat.visible_idx-1].com_time_up+sat_train_time < frontest_time) :
                sat.model_trained = 1
                sat.model_sended = 0
...
def vir_train_process(epochs_needed):
    global frontest_time
    ...
    while round < epochs_needed:
        ...
        for sat in satellites:
            if (sat.records[sat.visible_idx].start_tstamp <= frontest_time) & (sat.records[sat.visible_idx].stop_tstamp > frontest_time):
                sat.visited = 0
                sat.records[sat.visible_idx].start_time = frontest_time
        while len(MoRolla.model_up) < total_satellites_cnt:
            # 确定选择哪一颗卫星
            choosed_sat = choose_satellite()
            if choosed_sat == -1:
                # 没有选择到合适的，frontest_time给的不合适
                frontest_time = satellites[0].records[satellites[0].visible_idx+1].stop_tstamp
                continue
            ...
            if len(MoRolla.model_up) ==  total_satellites_cnt:
...
if __name__ == '__main__':
    ...
    dt1 = "2023-06-01 00:00:00"
    dt11 = time.strptime(dt1, "%Y-%m-%d %H:%M:%S")
    stamp1 = time.mktime(dt11) 

    vir_train_process(12)
