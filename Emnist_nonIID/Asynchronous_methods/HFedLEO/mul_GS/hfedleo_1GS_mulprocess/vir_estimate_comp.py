...
def single_process(args, dataset_train, dataset_test, dict_users_train, dict_users_test, net_glob, gid, fraction, idx,
                   w_glob):
...
def vir_comp_main(args, dataset_train, dataset_test, dict_users_train, dict_users_test, net_glob, fraction, gid):
    ...
    res = [pool.apply_async(single_process, args=(
        args, dataset_train, dataset_test, dict_users_train, dict_users_test, net_glob, gid, fraction, idx, w_glob)) for
           idx in idxs_users]
    pool.close()
    pool.join()
    for re in res:
        r = re.get()
        w_locals.append(r[0])
        loss_train_locals.append(r[1])
        acc_test_locals.append(r[2])
