...
def vir_train_process(epochs_needed, fraction, filename):
    ...
    with open(filename,'w') as f: # 如果filename不存在会自动创建， 'w'表示写数据，写之前会清空文件中的原有数据！
        while round < epochs_needed:
            ...
            while len(MoRolla.model_up) < math.ceil(total_satellites_cnt * fraction):
                # 确定选择哪一颗卫星
                ...
                if len(MoRolla.model_up) ==  math.ceil(total_satellites_cnt * fraction):
...
if __name__ == '__main__':
    ...
    vir_train_process(20, 0.50, "./output_info/participant_sats.txt")
